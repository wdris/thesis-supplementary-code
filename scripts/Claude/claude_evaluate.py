...
PREDICTION_FOLDER = os.path.join(PROJECT_ROOT, "results", "claude_output_mode4")
...
def get_gt_path(json_filename):
    """Derives GT file path from JSON filename."""
...
def normalize_text(text, mode):
    """
    Cleans text based on the selected normalization dimension.
    """
...
def calculate_comprehensive_metrics():
    print("Starting Multi-dimensional CER Evaluation...")
    print(f"Prediction Source: {PREDICTION_FOLDER}")
    
    json_files = glob.glob(os.path.join(PREDICTION_FOLDER, "*.json"))
    if not json_files:
        print("Error: No JSON files found in prediction folder.")
        return

    # define the 5 dimensions of evaluation
    metric_types = ['baseline', 'lower', 'nospace', 'alnum', 'alnumlowernospace']
    
    # initialize statistics for Micro Average calculation
    ...
    print("\n" + "="*70)
    print(f"Data Source: {os.path.basename(PREDICTION_FOLDER)}")
    print("-" * 70)
